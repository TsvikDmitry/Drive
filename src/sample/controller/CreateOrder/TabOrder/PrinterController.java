...
import sample.model.create_order.print.*;
import sample.model.create_order.print.paper.PaperDuplex;
import sample.model.create_order.print.paper.PaperDuplexDB;
import sample.model.create_order.print.paper.PaperPrice;
import sample.model.create_order.print.paper.PaperPriceDB;
...
public class PrinterController {
    ...
    public void rowPrinter(Integer id) {
        ...
        if (count!=0) {
            for (int i = 0; i <= count; i++) {
                ...
                if (PrinterTable.getSelectionModel().getSelectedItem().getIdPrinter()== id) {
                    break;
                }
                if (count == i) {
    ...
    private void PrinterPriceUpdate() throws Exception {
        try {
            ObservableList<PrinterPrice> printFormat = null;
                ...
                printFormat = PrinterPriceDB.selectPrinterPrice(getIdPrinter(),getIdFormat(), getCount());
    ...
    public void TextField() {
          ...
          PriceEditText.setTextFormatter(TextFieldFormatter.getDouble());
        ...
        ColumEdit.setTextFormatter(TextFieldFormatter.getItneger(1));
    ...
    public Integer getIdPrinter() {
        ...
        if(PrinterTable.getItems().size()!=0){
                txIdPrinter = PrinterTable.getSelectionModel().getSelectedItem().getIdPrinter();
        }
        return txIdPrinter;
    }
